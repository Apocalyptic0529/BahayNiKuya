...
| **Deep** | 5 + 2 gap-fill | 25 | 1-7 (all) | 15-30 min | Critical decisions, comprehensive reviews, multi-stakeholder analysis |
...
**Coverage checklist:** Alongside the focus areas, write down 5-8 concrete questions the final report must answer, derived from the research question. The checklist is the stopping criterion: every item must be answered or explicitly moved to Limitations before the report is written. Standard and Deep measure it in Phase 5; Quick skips Phase 5 and checks it directly at the Phase 7 quality gate, moving unanswered items straight to Limitations rather than searching further.
...
webSearch({ query: "<one full question>" })
...
- Fetch the 2-3 best URLs (chosen after all searches complete) with `webFetch({ url, query, path })` -- extraction and saved evidence in one call, no markdown printing or slicing
- Wrap fetches in try/catch and move to different URLs on failure (webFetch throws on unfetchable pages)
...
Size each subagent's task to the 4-6 searches and 2-3 fetches above: `subagent(...)` awaits time out at 300 seconds, and a job that runs past that gets backgrounded and must be collected separately with `waitForJob`.
...
- Measure the findings against the Phase 2 coverage checklist
...
Each search is one webSearch({ query: "<one full question>" }) call -- query is a REQUIRED string (there is no queries parameter) and results come back in .resultPages.
...
Write the report following these principles, then verify its citations before delivering.
...
| Source list complete | Every [N] citation maps to a numbered source with URL and saved evidence under research/sources/ |

| Citations verified | Major claims re-checked against saved evidence (Standard/Deep) |

| Coverage | Every Phase 2 checklist item answered or listed in Limitations |
...
Run all searches first, then fetch the 2-3 best pages, each with one call: webFetch({ url, query: "<your focus-area question>", path: "research/sources/market-[NN]-[page-slug].md" }). Log only the returned answer -- never print or slice markdown -- and savedTo confirms the full page is on disk. webFetch throws on unfetchable pages: try/catch and move to a different URL. Append snippets for sources you cite without fetching to research/sources/market-snippets.md.

Return findings using the structured template with Key Facts, Notable Claims, Source Quality Assessment, Gaps, and Sources (with saved paths).`;
const researchJobs = [
  subagent({
    name: "research-1",
    task: research1Task,
...
// Phase 7: Write the report, then verify major claims against the saved
// pages in research/sources/ before delivering
...
8. **Handle fetch failures gracefully** -- webFetch throws on unfetchable pages, so try/catch every call (Promise.allSettled for batches) and move to a different URL; never re-fetch the same failing one

9. **Prefer authoritative sources** -- when numbers conflict, prefer Tier 1 (government/multilateral) over Tier 2 (publications) over Tier 3 (blogs/promotional). SEO aggregators and forum threads are leads to primary sources, not evidence
10. **Flag stale data** -- note when key data points are older than 18 months on fast-moving topics
11. **Preserve evidence in the fetch itself** -- pass `query` and `path` on every research webFetch so the full page lands under research/sources/ and only the distilled answer enters context; never print or slice page markdown, and never save hand-written excerpts as if they were the page
...
- webFetch throws on some pages (JavaScript-heavy sites, paywalled content, dead links); mitigate with try/catch and alternative URLs
